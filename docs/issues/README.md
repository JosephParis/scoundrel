...
users.** Issues 01â€“25 came from a read-only audit of the codebase, API layer, and
...
| Spec | Project | Status |
|---|---|---|
| `screens.spec.js` | dev | 6 pass (1 known skip â€” issue 12) |
...
- `npm run test` â€” everything
- `npm run test:dev` / `npm run test:prod` â€” one project
...
3. Read the linked files before changing them â€” several issues note that the
...
launch, decide whether it needs its own entry or can share `0.4` â€” nothing has
shipped to users on `0.4` yet, so sharing is usually fine.

## Priorities

- **P0** â€” visible to every user on day one, or legally required. Do before launch.
- **P1** â€” data integrity and abuse. The link is public the moment you send it.
- **P2** â€” product decisions that need an explicit answer, not a default.
- **P3** â€” quality, performance, accessibility.
- **P4** â€” repo hygiene and doc accuracy.
...
01â€“03 are visible to every user on day one. 07 and 15 are what make the data you
collect from batch 1 worth acting on.

## Dependencies

```
05 (commit WIP) â”€â”€> everything else
                    â”‚
07 (auth writes) â”€â”€>â”œâ”€ 08 (moderation: needs a trusted accountId to block)
                    â”‚
10 (schema) â”€â”€â”€â”€â”€â”€â”€>â”œâ”€ 08 (needs a `blocked` column)
                    â”‚
15 (vitest) â”€â”€â”€â”€â”€â”€â”€>â”œâ”€ 24 (wire test:unit into CI)
09 (dedupe bug) â”€â”€â”€>â”˜    (09's acceptance criteria want a test)

23 (DESIGN.md) â”€â”€â”€â”€> 25 (rules copy) â”€â”€â”
                                       â”œâ”€ audit content together
12 (unhide tabs) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€>â”˜

11 (flag defaults) â”€â”€> 13 (measurement window)   settle flags before opening it
12 (library tab) â”€â”€â”€â”€> 11 (needs `library: true`)

20 (README) â”€â”€â”€â”€â”€â”€â”€> 22 (fold live setup notes in before archiving)
21 (.gitignore) â”€â”€â”€> 20 (.env.example must stay trackable)

03 (music) <â”€â”€â”€â”€â”€â”€â”€> 16 (audio cleanup: mourning-song.ogg may become gameover)
04 (manifest) â”€â”€â”€â”€â”€> 19 (indexing decision)
06 (privacy) <â”€â”€â”€â”€â”€> 18 (Google Fonts is a disclosure item)
```
...
`status: done`. It does not delete these files â€” they stay as the offline copy.
